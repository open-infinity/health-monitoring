...
public class MetricBoundariesHandler implements AbstractHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(MetricBoundariesHandler.class);

    @Override
    public void handle(HttpResponse response, QueryStringDecoder queryStringDecoder) throws Exception {
        String metricType = HttpUtil.get(queryStringDecoder, "metricType");
        LOGGER.debug("metricType: {}", metricType);

        MonitoringService monitoringService = new RrdMonitoringService();
        response.setContent(ChannelBuffers.copiedBuffer(monitoringService.getBoundaries(metricType).getBytes()));
    }
}
