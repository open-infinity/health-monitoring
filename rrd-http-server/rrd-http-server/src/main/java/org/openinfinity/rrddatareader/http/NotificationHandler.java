...
public class NotificationHandler implements AbstractHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(NotificationHandler.class);

    @Override
    public void handle(HttpResponse response, QueryStringDecoder queryStringDecoder) throws Exception {
        String startTimeStr = HttpUtil.get(queryStringDecoder, "startTime");
        String endTimeStr = HttpUtil.get(queryStringDecoder, "endTime");

        Long startTime = startTimeStr != null ? Long.parseLong(startTimeStr) : null;
        Long endTime = endTimeStr != null ? Long.parseLong(endTimeStr) : null;

        LOGGER.debug("Getting notifications for period {} : {}", startTime, endTime);

        MonitoringService monitoringService = new RrdMonitoringService();
        response.setContent(ChannelBuffers.copiedBuffer(monitoringService.getNotifications(startTime,
                endTime).getBytes()));
    }
